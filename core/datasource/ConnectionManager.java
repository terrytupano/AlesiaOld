...
public class ConnectionManager {
	 ...
	 */
	public static int getRowCount(String tn, String wc) throws Exception {
		int count = -1;
		String sql = "SELECT count(1) FROM " + tn;
		if (wc != null) {
			sql += " WHERE " + wc;
		}
		Statement sts = getConnection(tn).createStatement();
	 ...
	 * @throws AplicationException message for the user if any issue is found
	 */
	public static void testProfile(Record pr) throws AplicationException {
		...
		if (drn.equalsIgnoreCase("Oracle")) {
			if (!prps.contains("schema")) {
				throw new AplicationException("datas.msg01");
			}
		}
	}

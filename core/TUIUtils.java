...
public class TUIUtils {
	...
	public static JComponent getBackgroundPanel() {
		WebImage wi = new WebImage(TResourceUtils.getIcon("text")) {
			protected void paintComponent(Graphics g) {
				...
				g2d.setPaint(new LinearGradientPaint(0, 0, 0, getHeight(), new float[]{0f, 0.4f, 0.6f, 1f}, new Color[]{
						StyleConstants.bottomBgColor, Color.WHITE, Color.WHITE, StyleConstants.bottomBgColor}));
	...
	public static WebButtonGroup getButtonGroup() {
	 ...
	 */
	public static WebButton getWebButtonForToolBar(TAbstractAction taa) {
	...
	public static WebToggleButton getWebToggleButtonForToolBar(String cid) {
		WebToggleButton jb = new WebToggleButton();
		setToolTip("ttaction." + cid, jb);
		jb.setIcon(TResourceUtils.getSmallIcon(cid));
		jb.setDrawFocus(false);
		jb.setShadeWidth(0);
		jb.setPreferredWidth(46);
		return jb;
	...
	public static JEditorPane getMessageConsole() {
		JEditorPane jep = new JEditorPane();
		jep.setEditable(false);
		jep.setEditorKit(new StyledEditorKit());
		MessageConsole mc = new MessageConsole(jep);
	...
	/**
	 * TODO: this method is bad constructed. MODIFY
	 * 
	 * @param taa
	 * @param wbg
	 * @return
	 */
	public static WebToggleButton getWebToggleButtonForToolBar(TAbstractAction taa, WebButtonGroup wbg) {
		WebToggleButton jb = new WebToggleButton();
		jb.setToolTipText(null);
		jb.setDrawFocus(false);
		jb.setShadeWidth(0);
		...
		// if action is null because actions are dispached throught itemstate change
		if (taa != null) {
			jb.setAction(taa);
			jb.setName(TResourceUtils.getClassName(taa));
			TooltipManager.setTooltip(jb, (String) taa.getValue(TAbstractAction.SHORT_DESCRIPTION), TooltipWay.down);
